...
-- Backward-compatible projections retained for older local clients. New authenticated
-- clients write Chat through fs_append_chat_event; these tables remain read-only
-- compatibility surfaces so the locked local JSON contract is not broken.
...
create index if not exists fs_messages_job_idx on public.fs_messages (job_id, ts);
...
create or replace function public.fs_next_rec_seq() returns bigint
language plpgsql
security definer
set search_path = ''
as $fn$
...
begin
  ...
  insert into public.fs_meta(key, value) values ('reccounter', to_jsonb(1))
  on conflict (key) do update
    set value = to_jsonb(((public.fs_meta.value #>> '{}')::bigint) + 1),
        updated_at = now()
  returning (public.fs_meta.value #>> '{}')::bigint into n;
  return n;
end
$fn$;
...
declare t text;
begin
  foreach t in array array['fs_records','fs_chats','fs_users','fs_activities','fs_meta','fs_memberships','fs_messages','fs_approvals'] loop
...
revoke all on table public.fs_records,public.fs_chats,public.fs_users,
  public.fs_activities,public.fs_meta,public.fs_memberships,
  public.fs_messages,public.fs_approvals from public,anon,authenticated;
grant select on public.fs_chats to authenticated;
grant select on public.fs_messages,public.fs_approvals to authenticated;
...
revoke all on function public.fs_next_rec_seq() from public,anon,authenticated;
grant execute on function public.fs_next_rec_seq() to authenticated;

drop policy if exists legacy_messages_member_select on public.fs_messages;
create policy legacy_messages_member_select on public.fs_messages for select to authenticated
using (private.fs_is_active_member());
drop policy if exists legacy_approvals_member_select on public.fs_approvals;
create policy legacy_approvals_member_select on public.fs_approvals for select to authenticated
using (private.fs_is_active_member());
