...
**8.5 ห้ามส่งข้อมูลขึ้นเซิร์ฟเวอร์ก่อนที่จะเคยดึงลงมาสำเร็จสักครั้ง** — `sbCycle()` ต้องเรียก `sbPush()`
เมื่อ `sbReady === true` เท่านั้น และเมื่อยังไม่ `sbReady` ต้องบังคับดึงแบบ full เสมอ
ตอนเปิดหน้าเว็บที่ดึงไม่สำเร็จ ห้ามตั้ง `sbLastFull = Date.now()` (ไม่งั้นรอบถัดไปจะไปดึงแบบ delta
ซึ่งเติม `sbSnap` ไม่ครบ แล้วทุกงานจะดูเหมือน "ของใหม่" จนสำเนาเก่าใน localStorage ถูกดันขึ้นไปทับทั้งชุด)
อีกชั้นหนึ่งคือ `recTouched` — งานที่ไม่มีฐานเทียบใน `sbSnap` และไม่ได้ถูกแก้ในเครื่องนี้ **ห้ามส่งขึ้น**
ยกเว้นตอน seed เซิร์ฟเวอร์ที่ยืนยันแล้วว่าว่างเปล่า (`sbPush(true)`)
**เคสจริง: A00798 ถูกเขียนทับด้วยค่าก่อนแก้ 24 จุด และก่อนหน้านั้นงาน 17 ใบรวดเดียว — ทั้งคู่มาจากรูนี้**

**8.6 ด่านสิทธิ์ที่อยู่นอกแอป ต้องปิดประตูเฉพาะตอนที่มันตอบว่า "ไม่ให้" เท่านั้น** — `sbLoadWritePermission()`
ถาม `/sb/session` ว่าผู้ใช้คนนี้เขียนได้ไหม ซึ่งเป็นด่านของดีพลอยที่มี IAM **เท่านั้น**
ดีพลอยที่ไม่มีด่าน (ตอบ 404 / ไม่ใช่ JSON / ไม่มีฟิลด์ `canEdit`) ต้องถือว่า **ไม่มีใครมาห้าม** แล้วทำงานตามปกติ
เรียกด่านไม่ได้ชั่วคราว = คงคำตอบครั้งล่าสุด (`sbIamSeen`) ไม่ใช่แปลว่าถูกปฏิเสธ
และคำขอต้องมี timeout เสมอ ด่านที่ค้างห้ามถ่วงการเปิดแอป
**เคสจริง: เวอร์ชัน 2026-09-20.1 ตั้ง `sbCanEdit = false` เมื่อเรียกด่านไม่สำเร็จ
แต่ดีพลอยจริงไม่มี `/sb/session` ทุกคนจึงกลายเป็นอ่านอย่างเดียวเงียบ ๆ
ไม่มีอะไรถูกบันทึกขึ้นเซิร์ฟเวอร์เลย ป้ายสถานะยังขึ้นเขียวว่า "ซิงก์แล้ว"
และเลขงานไม่ถูกจองจากเซิร์ฟเวอร์ → เสี่ยงสองเครื่องได้รหัสงานซ้ำกัน**
