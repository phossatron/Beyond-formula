...
**8.4 ตารางสูตรต้องกันค่าที่ "พิมพ์ค้าง" หาย** — ช่องในตารางสูตรบันทึกตอน `onchange` (คือตอนออกจากช่อง)
ถ้าหน้าจอถูกวาดใหม่ระหว่างนั้น element เดิมหายไปทั้งตัว สิ่งที่พิมพ์ค้างหายเงียบ ๆ โดยไม่มี event ให้จับ
ทุกจุดที่วาดทับ `#chatContent` / `#modContent` ต้องคร่อมด้วย `fxSnapFocus()` → `fxRestoreFocus()` เสมอ
และ**ห้ามวาดหน้าใหม่แบบ synchronous จากใน handler ของ `onchange`** (ตอนนั้นเบราว์เซอร์ยังย้ายโฟกัสไม่เสร็จ
ช่องที่ผู้ใช้กด Tab เข้าไปจะถูกทิ้ง แล้วสิ่งที่พิมพ์ต่อหายหมด) — ใช้ `chatSaveFormula(r, msg, true)` ให้เลื่อนไปวาดทีหลัง
ปุ่ม "บันทึกสูตร" (`saveChatFormula()` / `saveModifyFormula()`) อ่านค่าจาก DOM ตรง ๆ จึงบันทึกได้โดยไม่ต้องออกจากช่องก่อน — ห้ามถอดออก
